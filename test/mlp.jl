...
for lay in [:tapex, :bpex]
    @time g, W, teacher, E, stab = DeepMP.solve(α=0.2, K=[1001,7,1]
                , layers=[:tap,lay]
...
## @time g, W, teacher, E, stab = DeepMP.solve(K=[301,5,1] ,layers=[:bpreal,:bpex]
...
for lay in [:tapex] #TODO  :bpex non ce la fa
    @time g, W, teacher, E, stab = DeepMP.solve(α=0.2, K=[401,21,3,1]
                , layers=[:tap,lay,lay]
...
# @time g, W, teacher, E, stab = DeepMP.solve(α=0.25, K=[301,21,11,3,1]
...
# @time g, W, teacher, E, stab = DeepMP.solve(α=0.2, K=[301,21,11,3,1]
...
# @time g, W, teacher, E, stab = DeepMP.solve(α=0.15, K=[301,21,11,3,1]
#                    , layers=[:tap,:tap,:tapex,:bpex]
